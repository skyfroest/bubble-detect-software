...
def createModel(parameters):
    ...
    '''network = residual_block(network, 1,  parameters.CNN1FeatureSize, downsample=False, activation='relu', regularizer="L2")'''
    ...
    if parameters.CNN4Size > 0:
        network = conv_2d(network, parameters.CNN3FeatureSize, parameters.CNN4Size, activation='relu', regularizer="L2")#4. CNN
        network=batch_normalization(network)
    if parameters.CNN4Size > 0:
